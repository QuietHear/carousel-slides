let Shutter = function (elem, options) {
    this.defaults = {
        shutterW: 1200,
        shutterH: 500,
        isAutoPlay: false,
...
Shutter.prototype = {
    ...
    toggleMove: function (command, index) {
        ...
        $('.bigCarousel .indicator .active').removeClass('active');
        $('.bigCarousel .indicator li[data-place=' + index + ']').addClass('active');
        this.shutterTitle = this.$shutterItem.eq(index).attr('data-shutter-title');
    },
    initalShutter: function () {
        let $curElem = this.$shutterItem.eq(this.curDisplay),
            $nearlyElem = this.$shutterItem.not($curElem);
    ...
    inital: function (elem) {
        let self = this;
        this.$shutter = elem;
        this.$shutterItem = this.$shutter.find('.list a');
        this.$prevBtn = this.$shutter.find('.controls.prv-btn');
        this.$nextBtn = this.$shutter.find('.controls.next-btn');
        this.$shutterNav = this.$shutter.find('.indicator li');
        this.$shutterDesc = this.$shutter.find('.tit');
        ...
        if (this.begin) {
            this.begin = false;
            this.$shutterDesc.text(this.$shutterItem.eq(this.curDisplay).attr('data-shutter-title'))
        }
        // 上一个事件
        ...
        // 指示器事件
        this.$shutterNav.bind('click', function () {
            let nowIndex = parseInt($(this).attr('data-place'));
            if (self.curDisplay > nowIndex) {
                self.toggleMove('prev', nowIndex);
            }
            else if (self.curDisplay < nowIndex) {
                self.toggleMove('next', nowIndex);
            }
        });
        if (this.opts.fullPage) {
    ...
    recovery: function (target, cur, index, backup, interval) {
        ...
        setTimeout(function () {
            ...
            self.$shutterDesc.animate({bottom: 0}).text(self.shutterTitle);
            self.b_stop = true;
        }, interval);
    },
    gridWhole: function (index, showNum) {
        let self = this,
            $curElem = this.$shutterItem.eq(this.curDisplay),
            $targetElem = this.$shutterItem.eq(index),
            backup = $curElem.html(),
            $createElem = $('<div class="created"></div>'),
            movingVal = 0;
    ...
    gridTop: function (index, showNum) {
        ...
        $curElem.find('img').fadeOut();
        for (let i = 0; i < 12; i++) {
            let $createElem = $('<div class="created"></div>');
            $createElem.html(backup).css({
                position: 'absolute',
                zIndex: 20,
...
$.fn.shutter = function (options) {
    this.each(function () {
        let shutter = new Shutter($(this), options);
    });
};
